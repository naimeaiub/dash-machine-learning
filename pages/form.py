...
modelo = joblib.load('machine_learning/model_xgboost.pkl')
medianas = joblib.load('machine_learning/median.pkl')

formulario = dbc.Container([
    html.P("Preencha as informações abaixo e clique em prever para rodar o modelo",
           className="text-center mb-5 custom-subtitle"),
    dbc.Row([
            dbc.Col([
                dbc.CardGroup([
                    dbc.Label("Idade"),
            ...
            dbc.Col([
                ...
                dbc.CardGroup([
                    ...
                    dbc.Input(id='oldpeak', type='number',
                              placeholder='Digite a depressão do segmento ST induzida por exercício'),
                ], className='mb-3 custom-input'),
                # slope
                dbc.CardGroup([
...
layout = html.Div([
    html.H1("Previsão de doença cardíaca",
            className="text-center mt-5 custom-title"),
    formulario,
...
def prever_doenca(n_clicks, idade, sexo, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal):
    ...
    # oldpeak é float
    entradas_usuario['oldpeak'] = entradas_usuario['oldpeak'].astype(
        np.float64)
    ...
    if previsao == 1:
        mensagem = "Possui doença cardíaca"
        cor_do_alerta = 'danger'
    else:
        mensagem = "Não possui doença cardíaca"
        cor_do_alerta = 'light'
